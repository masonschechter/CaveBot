...
# con = sqlite3.connect("CaveBot_DEV.db")
...
class Currency(commands.Cog):
    ...
    def getBalance(self, user: discord.Member):
        ...
        sql_select = "SELECT balance FROM High_Rollers WHERE discord_id = %s"
        uid = str(user.id)
        cur.execute(sql_select, (uid,))
        result = cur.fetchone()

        if not result: ## first time trying to gamble/check nugs
            sql_insert = "INSERT INTO High_Rollers (discord_id, balance) VALUES (%s, %s)"
            data = (uid, DEFAULT_BALANCE)
